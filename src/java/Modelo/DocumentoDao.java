...
public class DocumentoDao {
    ...
    public int agregar(Documento docu){ 
        String sql="insert into documento(titulo_doc, descripcion_doc,finicio_doc,documento_departamento, documento_tipodoc, documento_usuario)values(?,?,?,?,?,?)";
        try {
            con=cn.Conexion();
            ps=con.prepareStatement(sql);
                ...
                ps.setString(3, docu.getFinicio_doc());                 
    ...
    public Documento listarId(int id){
        ...
        try {
            ...
            while (rs.next()) {
                ...
                docu.setDoc_dep(rs.getInt(5));   
                docu.setDoc_tipodoc(rs.getInt(6));
                docu.setDoc_user(rs.getInt(7));               
    ...
    public int actualizar(Documento docu){
       String sql="update documento set titulo_doc=?, descripcion=?, finicio_doc=?, documento_departamento=?, documento_tipodoc=?, documento_usuario=? where id_documento = ?";
        try {
            con=cn.Conexion();
            ps=con.prepareStatement(sql);
